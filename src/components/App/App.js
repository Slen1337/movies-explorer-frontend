...
import InfoTooltip from "../Form/InfoTooltip/InfoTooltip";
...
export default function App() {
  ...
  function handleSearch(checked) {
    ...
    const filteredMovies =
      location === "/movies" ? initialMovies : initialSavedMovies;

    if (word.length > 0) {
      sortedMovies = filteredMovies.filter((movie) =>
        JSON.stringify(movie).toLowerCase().includes(word.toLowerCase())
      );
      sortedMovies.length === 0 && setMoviesMessage("Ничего нет");
      if (checked) {
        location === "/movies"
  ...
  function handleSaveMovie(movie) {
    api
      ...
      .catch((err) => {
        setInfoMessage(errors(err));
        setMoviesMessage("Нет сохраненных фильмов");
      });
  }
