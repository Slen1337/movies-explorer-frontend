    ...
    <ul className="user__menu">
      <li className="user__menu-item">
        <Link
          ...
          className="user__button-signup"
        >
          {loggedIn ? "Аккаунт" : "Регистрация"}
        </Link>
      </li>
      <li
        className={
          loggedIn ? "user__menu-item user__menu-item_login" : "user__menu-item"
        }
      >
        <Link
          to={loggedIn ? "/profile" : "signin"}
          className={loggedIn ? "user__link" : "user__link user__link_login"}
        >
          {loggedIn ? (
